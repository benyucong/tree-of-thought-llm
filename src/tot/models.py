...
def gpt(prompt, model=default_model, temperature=0.7, max_tokens=1000, n=1, stop=None) -> list:
    messages = [{"role": "user", "content": prompt}]
    return chatgpt(messages, model=model, temperature=temperature, max_tokens=max_tokens, n=n, stop=stop)
    
def chatgpt(messages, model=default_model, temperature=0.7, max_tokens=1000, n=1, stop=None) -> list:
